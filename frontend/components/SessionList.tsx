      ...
      {sessions.map((session) => (
        ...
        >
          <div className="flex justify-between items-start mb-4">
            {showSummary ? (
              <div
                className={`px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase tracking-wider ${getEmotionColor(session.summary.emotion)}`}
              >
            ...
            <div className="flex items-center gap-3">
              ...
              {onRequestCounsellor &&
                ['CRITICAL', 'BAD'].includes(session.summary?.emotion || '') && (
                  <button
                    type="button"
                    onClick={(event) => {
                      event.preventDefault();
          ...
          <div className="mt-auto space-y-2 text-xs text-slate-500">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Clock size={14} />
                <span>
                  {resolveSessionDate(session)?.toLocaleDateString() || 'Date unavailable'}
                </span>
              </div>
