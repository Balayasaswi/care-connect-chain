const mode = String(process.argv[2] || "skip").trim().toLowerCase();
...
if (mode === "amoy") {
  const rpcUrl = String(process.env.BLOCKCHAIN_RPC_URL || "https://polygon-amoy.g.alchemy.com/v2/YOUR_KEY").trim();
  const privateKey = String(process.env.BLOCKCHAIN_PRIVATE_KEY || "0xYOUR_TEST_WALLET_PRIVATE_KEY").trim();
  const contract = String(process.env.CID_REGISTRY_CONTRACT_ADDRESS || "0xYOUR_DEPLOYED_CONTRACT").trim();
