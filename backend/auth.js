...
export function exportUsersCsv() {
  ...
  const rows = db
    .prepare(
      'SELECT id, username, email, institution_access_code, institution_college_code, created_at FROM users ORDER BY created_at DESC',
    )
    .all();
  const header = [
    'id',
    'username',
    'email',
    'institution_access_code',
    'institution_college_code',
    'created_at',
  ].join(',');
  const lines = rows.map((r) =>
    [r.id, r.username, r.email, r.institution_access_code, r.institution_college_code, r.created_at]
      .map(csvEscape)
      .join(','),
  );
...
export function appendBlockchainCsv(payloadOrChainId, address, txHash, timestamp) {
  const payload =
    typeof payloadOrChainId === 'object'
      ? payloadOrChainId
      : {
  ...
  try {
    ...
    if (normalizedCollegeCode) {
      ...
      if (!institution) {
        return {
          success: false,
          error: 'Invalid college code. Ask your institution for the correct college ID.',
        };
      }
    ...
    const resolvedCollegeCode = normalizeCollegeCode(
      institution?.college_code || normalizedCollegeCode,
    );
...
export function getUserByEmail(email) {
  const stmt = db.prepare(
    'SELECT id, username, email, institution_access_code, institution_college_code, created_at FROM users WHERE email = ?',
  );
...
export function getUserById(userId) {
  const stmt = db.prepare(
    'SELECT id, username, email, institution_access_code, institution_college_code, created_at FROM users WHERE id = ?',
  );
  return stmt.get(userId);
}

export function getUsersByInstitutionCollegeCode(collegeCode) {
  const normalized = String(collegeCode || '')
    .trim()
    .toUpperCase();
  ...
  return db
    .prepare(
      "SELECT id, username, email FROM users WHERE UPPER(COALESCE(institution_college_code, '')) = ?",
    )
    .all(normalized);
}

export function getUsersByCounsellorInstitution(collegeCode) {
  const normalizedCollegeCode = String(collegeCode || '')
    .trim()
    .toUpperCase();
...
export function addGuardian(studentId, guardianData) {
  try {
    const { guardian_name, guardian_email, guardian_phone, relationship, guardian_password } =
      guardianData;
...
export async function registerCounsellor(
  counsellorEmail,
  counsellorPassword,
  crrNumber,
  organization,
  ...
  try {
    ...
    if (normalizedCrr) {
      ...
      if (
        existingByCrr &&
        String(existingByCrr.counsellor_email || '').toLowerCase() !==
          String(counsellorEmail || '').toLowerCase()
      ) {
        return {
          success: false,
          error: 'CRR Number is already registered with another counsellor',
        };
      }
    }
    ...
    try {
      ...
      console.warn('Counsellors CSV export failed:', e.message);
    }
    return { success: true };
  } catch (error) {
    return { success: false, error: error.message };
...
export async function loginCounsellor(loginId, counsellorPassword, { collegeCode } = {}) {
  try {
    ...
    const records = byEmail
      ? db
          .prepare('SELECT * FROM counsellors_global WHERE LOWER(counsellor_email) = LOWER(?)')
          .all(rawLoginId)
    ...
    const filtered = records.filter((record) => {
      return (
        String(record.college_code || '')
          .trim()
          .toUpperCase() ===
        String(normalized.collegeCode || '')
          .trim()
          .toUpperCase()
      );
    });
    ...
    if (filtered.length > 1) {
      return {
        success: false,
        error: 'Multiple counsellor records match this CRR. Please login with email.',
      };
    }
...
export async function registerInstitution(
  ...
  try {
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      const existing = db
        .prepare('SELECT 1 FROM institutions_global WHERE college_code = ?')
        .get(candidate);
      if (!existing) {
        collegeCode = candidate;
        break;
      }
    }
    if (!collegeCode)
      return { success: false, error: 'Could not generate unique college code, please retry' };
    ...
    try {
      ...
      console.warn('Institutions CSV export failed:', e.message);
    }
...
export function createNetworkConnectionRequest(studentId, actorId, actorRole, relationType) {
  ...
  try {
    ...
    if (String(error.message || '').includes('UNIQUE constraint')) {
      const existing = db
        .prepare(
          'SELECT * FROM network_connections WHERE student_id = ? AND actor_id = ? AND actor_role = ?',
        )
...
export function getNetworkConnectionById(connectionId) {
  return db
    .prepare('SELECT * FROM network_connections WHERE id = ?')
    .get(String(connectionId || '').trim());
}
...
export function listNetworkConnectionsForStudent(studentId, statuses = []) {
  ...
  return db
    .prepare(
      `SELECT * FROM network_connections WHERE student_id = ? AND status IN (${placeholders}) ORDER BY created_at DESC`,
    )
...
export function listNetworkConnectionsForActor(actorId, actorRole, statuses = []) {
  ...
  if (!validStatuses.length) {
    return db
      .prepare(
        'SELECT * FROM network_connections WHERE actor_id = ? AND actor_role = ? ORDER BY created_at DESC',
      )
  ...
  return db
    .prepare(
      `SELECT * FROM network_connections WHERE actor_id = ? AND actor_role = ? AND status IN (${placeholders}) ORDER BY created_at DESC`,
    )
...
export function createCounsellorRequest(payload) {
  ...
  const sessionEmotion = String(payload?.sessionEmotion || '')
    .trim()
    .toUpperCase();
  ...
  const requestedByRole = String(payload?.requestedByRole || '')
    .trim()
    .toLowerCase();
  const requestedByEmail = String(payload?.requestedByEmail || '')
    .trim()
    .toLowerCase();
  ...
  if (!studentId || !urgency || !requestedByEmail || !requestedByRole) {
    return {
      success: false,
      error: 'student_id, urgency, requested_by_role, and requested_by_email are required',
    };
  }
...
export function updateCounsellorRequestStatus(requestId, status, handledBy) {
  ...
  const normalizedHandledBy = String(handledBy || '')
    .trim()
    .toLowerCase();
  ...
  const existing = db
    .prepare('SELECT * FROM counsellor_requests WHERE id = ?')
...
export function createCounsellorSchedule(payload) {
  const studentId = String(payload?.studentId || '').trim();
  const counsellorEmail = String(payload?.counsellorEmail || '')
    .trim()
    .toLowerCase();
  ...
  if (!studentId || !counsellorEmail || !scheduledFor) {
    return {
      success: false,
      error: 'student_id, counsellor_email, and scheduled_for are required',
    };
  }
