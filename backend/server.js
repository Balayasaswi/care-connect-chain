...
import {
  ...
  registerGuardian,
  loginGuardian,
  readIpfsEntriesByStudent,
  readBlockchainEntriesByStudent,
  registerCounsellor,
  loginCounsellor,
  registerInstitution,
  loginInstitution,
  getInstitutionByCollegeCode,
  getGuardiansByEmail,
  readAllIpfsEntries,
  getCounsellorByEmail,
  getUsersByInstitutionCollegeCode,
  getUsersByCounsellorInstitution,
  createNetworkConnectionRequest,
  getNetworkConnectionById,
  updateNetworkConnectionStatus,
  listNetworkConnectionsForStudent,
  listNetworkConnectionsForActor,
  deleteNetworkConnection,
  createCounsellorRequest,
  listCounsellorRequestsForCounsellor,
  updateCounsellorRequestStatus,
  createCounsellorSchedule,
  listCounsellorSchedules,
  listCounsellorSchedulesForStudent,
  markCounsellorScheduleReadByStudent,
  saveSessionArchive,
...
const IPFS_PROVIDER = String(process.env.IPFS_PROVIDER || 'local')
  .trim()
  .toLowerCase();
const IPFS_API_BASE = String(process.env.IPFS_API_BASE || 'http://127.0.0.1:5001/api/v0')
  .trim()
  .replace(/\/$/, '');
const IPFS_GATEWAY_BASE = String(process.env.IPFS_GATEWAY_BASE || 'http://127.0.0.1:8080/ipfs')
  .trim()
  .replace(/\/$/, '');
const IPFS_READ_GATEWAYS = String(
  process.env.IPFS_READ_GATEWAYS || `${IPFS_GATEWAY_BASE},${PINATA_GATEWAY_BASE}`,
)
  .split(',')
...
const CID_REGISTRY_CONTRACT_ADDRESS = String(
  process.env.CID_REGISTRY_CONTRACT_ADDRESS || process.env.CID_CONTRACT_ADDRESS || '',
).trim();
const BLOCKCHAIN_CHAIN_ID = parsePositiveInt(process.env.BLOCKCHAIN_CHAIN_ID, 0);
const BLOCKCHAIN_OPTIONAL =
  String(process.env.BLOCKCHAIN_OPTIONAL || 'true')
    .trim()
    .toLowerCase() !== 'false';
...
async function storeCidOnChain({ cid, studentId = '', sessionId = '' }) {
  ...
  const existingEntries = String(studentId || '').trim()
    ? readBlockchainEntriesByStudent(studentId).filter(
        (entry) => String(entry.cid || '') === String(cid),
      )
    : [];
...
function buildLocalCounsellorReply(message) {
  ...
  const distressPattern =
    /\b(sad|low|depressed|anxious|anxiety|panic|stressed|stress|overwhelmed|scared|angry|hurt|lonely|bad)\b/;
  ...
  try {
    ...
    if (!username || !email || !password || (!access_code && !college_code)) {
      return res
        .status(400)
        .json({
          error: 'username, email, password, and college_code (or access_code) are required',
        });
    }
...
app.post('/api/network/connect-request', (req, res) => {
  try {
    ...
    const actorRole = String(actor_role || '')
      .trim()
      .toLowerCase();
  ...
  try {
    ...
    if (!guardian_email || !guardian_password || !student_email || !relationship) {
      return res
        .status(400)
        .json({
          error: 'guardian_email, guardian_password, student_email, and relationship are required',
        });
    }
...
app.post('/api/guardian/login', async (req, res) => {
  try {
    ...
    if (guardians.length > 1) {
      return res
        .status(409)
        .json({ error: 'Guardian email is linked to multiple students. Contact admin support.' });
    }
...
app.get('/api/guardian/summaries', async (req, res) => {
  try {
    const studentId = String(req.query.student_id || '').trim();
    const guardianEmail = String(req.query.guardian_email || '')
      .toLowerCase()
      .trim();
...
app.get('/api/sessions', async (req, res) => {
  try {
    ...
    const guardianEmail = String(req.query.guardian_email || '')
      .toLowerCase()
      .trim();
    const collegeCode = String(req.query.college_code || '')
      .trim()
      .toUpperCase();
    ...
    const sessionIds = new Set(
      sessions.map((session) => String(session.id || '').trim()).filter(Boolean),
    );
    const cidsInSessions = new Set(
      sessions.map((session) => String(session?.ipfs?.cid || '').trim()).filter(Boolean),
    );
    ...
    for (const failedEntry of failedEntries) {
      ...
      sessions.push({
        ...
        summary: {
          ...
          emotion: 'NEUTRAL',
          summary:
            'Session saved, but its IPFS content is currently unavailable from configured gateways.',
        },
        history: [],
        status: 'completed',
        ipfs: {
...
app.post('/api/counsellor/register', async (req, res) => {
  try {
    const { counsellor_email, counsellor_password, crr_number, organization, college_code } =
      req.body || {};
    const resolvedCollegeCode = String(college_code || '')
      .trim()
      .toUpperCase();
    if (!counsellor_email || !counsellor_password || !crr_number) {
      return res
        .status(400)
        .json({ error: 'counsellor_email, counsellor_password, and crr_number are required' });
    }
...
app.post('/api/counsellor/login', async (req, res) => {
  try {
    ...
    const result = await loginCounsellor(String(login_id).trim(), String(counsellor_password), {
      collegeCode: resolvedCollegeCode,
    });
...
app.get('/api/counsellor/students', (req, res) => {
  try {
    const counsellorEmail = String(req.query.counsellor_email || '')
      .toLowerCase()
      .trim();
    if (!counsellorEmail) {
...
app.post('/api/counsellor/requests/:requestId/create-session', (req, res) => {
  try {
    ...
    const counsellorEmail = String(req.body?.counsellor_email || '')
      .toLowerCase()
      .trim();
    ...
    const updateResult = updateCounsellorRequestStatus(
      requestId,
      'session_created',
      counsellorEmail,
    );
...
app.post('/api/counsellor/schedules', (req, res) => {
  try {
    const { student_id, counsellor_email, scheduled_for, urgency, notes, source_request_id } =
      req.body || {};
...
app.post('/api/institution/register', async (req, res) => {
  try {
    ...
    if (!institution_email || !institution_password) {
      return res
        .status(400)
        .json({ error: 'institution_email and institution_password are required' });
    }
    if (!resolvedCode) {
...
app.post('/api/ipfs/pin-session', async (req, res) => {
  try {
    ...
    if (!sessionId || !userId || !summary || !Array.isArray(history)) {
      return res
        .status(400)
        .json({ error: 'sessionId, userId, summary, and history are required' });
    }
    ...
    const pinResult = await pinJsonToIpfs(payload, `care-connect-${sessionId}`, {
      name: `care-connect-${sessionId}`,
    });
    const cid = pinResult.cid;
...
app.post('/api/blockchain/record', (req, res) => {
  try {
    const { chainId, address, txHash, timestamp, userId, sessionId, cid, contractAddress } =
      req.body || {};

    if (!chainId || !address || !txHash || !timestamp) {
      return res
        .status(400)
        .json({ error: 'chainId, address, txHash, and timestamp are required' });
    }
